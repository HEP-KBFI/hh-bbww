  ...
  , option_(kOption_undefined)
{
  ...
  central_or_shiftOptions_["memProb_signal"] = { "central" };
  central_or_shiftOptions_["memProbErr_signal"] = { "central" };
  central_or_shiftOptions_["memProb_background"] = { "central" };
...
EvtHistManager_hh_bb2l::bookHistograms(TFileDirectory & dir)
{
  ...
  if ( option_ == kOption_memEnabled ) {
    histogram_memProb_signal_        = book1D(dir, "memProb_signal",         "memProb_signal",         200, -50.,  +50.);
    histogram_memProbErr_signal_     = book1D(dir, "memProbErr_signal",      "memProbErr_signal",      200, -50.,  +50.);
    histogram_memProb_background_    = book1D(dir, "memProb_background",     "memProb_background",     200, -50.,  +50.);
				       ...
				       double evtWeight)
{
  ...
  if ( option_ == kOption_memEnabled ) {
    assert(memResult);
    fillWithOverFlow(histogram_memProb_signal_,        memResult->getProb_signal(),        evtWeight, evtWeightErr);            
    fillWithOverFlow(histogram_memProbErr_signal_,     memResult->getProbErr_signal(),     evtWeight, evtWeightErr);    
