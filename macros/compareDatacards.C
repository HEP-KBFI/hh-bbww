               ...
               double legendY0 = 0.72)
{
  ...
  //ref.histogram_->SetMaximum(2*ref.histogram_->GetMaximum());
  ...
  for(const HistogramEntry & test: tests)
  {
    if(test.histogram_)
    {
      ...
      if(histogram_div_ref)
      {
        ...
        histogram_div_ref->SetMinimum(-0.30);
        histogram_div_ref->SetMaximum(+0.30);
...
TH1* loadHistogram(TFile* inputFile, const std::string& histogramName)
{
  ...
  std::string hist = Form("HH_1l_0tau/%s", histogramName.data());
  //std::string hist = Form("%s", histogramName.data());
  TH1* histogram = dynamic_cast<TH1*>(inputFile->Get(hist.data()));
              ...
              const std::string & header)
{
  ...
  if (histogram_test) {
    ...
    integral_test = compIntegral(histogram_test);
                          ...
                          bool useFRsys)
{
  const std::vector<std::string> processes = {
    "Fakes"//"TT",// "ggHH_kl_1_kt_1_hbbhww"//"ST", "Other_bbWW", "WJets", "TT",
    //"ttH_hww", "ttH_hzz", "ttH_htt", "ttH_hbb",
    //"tHq_hzz", "tHq_htt", "tHq_hbb", "tHq_hgg",
    //"tHW_hww", "tHW_hzz", "tHW_htt", "tHW_hbb", "tHW_hgg",
    //"WH_hww", "WH_hzz", "WH_htt", "WH_hbb", "WH_hgg",
    //"ZH_hww", "ZH_hzz", "ZH_htt", "ZH_hbb", "ZH_hgg",
    //"qqH_hww", "qqH_hzz", "qqH_htt", "qqH_hbb", "qqH_hgg",
    //"ggH_hww", "ggH_hzz", "ggH_htt", "ggH_hbb", "ggH_hgg",
    /*"qqHH_CV_1p5_C2V_1_kl_1_hbbhww", "qqHH_CV_1p5_C2V_1_kl_1_hbbhtt", "qqHH_CV_1p5_C2V_1_kl_1_hbbhzz", 
    "ggHH_kl_5_kt_1_hbbhtt", "ggHH_kl_5_kt_1_hbbhww", "ggHH_kl_5_kt_1_hbbhzz", 
    "ggHH_kl_1_kt_1_hbbhtt", "ggHH_kl_1_kt_1_hbbhww", "ggHH_kl_1_kt_1_hbbhzz",
    "ggHH_kl_2p45_kt_1_hbbhtt", "ggHH_kl_2p45_kt_1_hbbhww", "ggHH_kl_2p45_kt_1_hbbhzz",
    "ggHH_kl_0_kt_1_hbbhtt", "ggHH_kl_0_kt_1_hbbhww", "ggHH_kl_0_kt_1_hbbhzz",
    "qqHH_CV_1_C2V_1_kl_1_hbbhww", "qqHH_CV_1_C2V_1_kl_1_hbbhtt", "qqHH_CV_1_C2V_1_kl_1_hbbhzz",
    "qqHH_CV_0p5_C2V_1_kl_1_hbbhww", "qqHH_CV_0p5_C2V_1_kl_1_hbbhtt", "qqHH_CV_0p5_C2V_1_kl_1_hbbhzz",
    "qqHH_CV_1_C2V_1_kl_2_hbbhww", "qqHH_CV_1_C2V_1_kl_2_hbbhtt", "qqHH_CV_1_C2V_1_kl_2_hbbhzz",
    "qqHH_CV_1_C2V_1_kl_0_hbbhww", "qqHH_CV_1_C2V_1_kl_0_hbbhtt", "qqHH_CV_1_C2V_1_kl_0_hbbhzz",
    "qqHH_CV_1_C2V_2_kl_1_hbbhww", "qqHH_CV_1_C2V_2_kl_1_hbbhtt", "qqHH_CV_1_C2V_2_kl_1_hbbhzz",
    "qqHH_CV_1_C2V_0_kl_1_hbbhww", "qqHH_CV_1_C2V_0_kl_1_hbbhtt", "qqHH_CV_1_C2V_0_kl_1_hbbhzz"*/
  };

  const std::vector<std::string> sysShifts_prefix = {
    //"pdf_fit3"
    "CMS_bbwwsl_Clos_m_shape_HH_resolved_2b_2016"
  };
  std::vector<std::string> sysShifts_common;
  for (auto sys: sysShifts_prefix) {
    sysShifts_common.push_back(Form("%sUp", sys.data()));
    sysShifts_common.push_back(Form("%sDown", sys.data()));
  }
  sysShifts_common.push_back("");
  ...
  for(const std::string & process: processes)
  {
    ...
    for(const std::string & central_or_shift: sysShifts)
    {
     ...
     const std::string histogramName_shifted = Form("%s_%s", process.data(), central_or_shift.data());
      ...
      //if(is_central) makePlot(entry, Form("%s/%s", outputFilePath_process_sys.data(), outputFileName.data()), histogramName, header);
      if(! is_central)
      {
...
void compareDatacards()
{
   ...
   //const std::string ref_path  = "/home/snandan/cmssw10/CMSSW_10_2_13/src/CombineHarvester/ttH_htt/datacard_SM/2016/nonresNLO/datacards_rebined/";
   const std::string ref_path  = "/home/snandan/datacard_wfullsyt_moretopsyst_pdfshape_pol_3/2016/res/spin0/300/datacards_rebined/";
    const std::string test_path = "";//"/home/karl/ttHAnalysis/2017/2018Aug06/datacards/3l_1tau/";
    std::vector<ComparisonEntries> entries;
    std::string evtcat [] = {"HH_resolved_2b"};//, "HH_resolved_2b"};//, "W_Other", "TT_H_resolved", "TT_H_boosted"};
    for ( const auto & cat : evtcat) 
      //for(const std::array<std::string, 3> & var: discriminatingVariables_1l_2tau)
    {
      std::string nbins = (cat == "HH_boosted") ? "18" : ((cat=="HH_resolved_1b") ? "20" : ((cat=="HH_resolved_2b") ? "20" : "5"));
      double sf_ref = 1.;
      const InputFileEntry ref = {ref_path,  Form("datacard_hh_bb1l_LBN_%s_MVAOutput_300_spin0_%sbins_quantiles_res_2016.root", cat.c_str(), nbins.c_str()), "Official dcards", sf_ref};
      //const InputFileEntry ref = {ref_path,  "addSystFakeRates_hh_bb1l_LBN_HH_resolved_2b_MVAOutput_750_spin0.root", "Official dcards", sf_ref};
      double sf_test = 1.;
      const InputFileEntry test = {test_path, Form("addSystFakeRates_%s.root", cat.c_str()), "New dcards", sf_test};
      
      const std::string outputFilePath = Form("/home/snandan/hh_Analysis/CMSSW_11_1_2/src/hhAnalysis/bbww/plots_%s", cat.c_str());
      entries.push_back({ref, {}, {}, outputFilePath, "MVAOutput", ""});
    }
