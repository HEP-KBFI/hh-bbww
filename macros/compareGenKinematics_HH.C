...
void compareGenKinematics_HH()
{
  ...
  std::string directoryName  = "hh_bb1l_Tight/sel/genKinematics_HH";
  ...
  observables.push_back("gen_mHH");
  observables.push_back("gen_absCosThetaStar");

  std::map<std::string, std::string> xAxisTitles; // key = observable
  xAxisTitles["gen_mHH"] = "gen. m_{HH} [GeV]";
  xAxisTitles["gen_absCosThetaStar"] = "gen. |cos(#theta*)|";
  ...
  for ( std::vector<std::string>::const_iterator period = periods.begin();
    ...
    for ( std::vector<std::string>::const_iterator observable = observables.begin();
      ...
      std::string histogramName_LO_woHHReweighting_full = Form("%s/%s/%s", directoryName.data(), subdirectoryName_LO_woHHReweighting.data(), observable->data());
      ...
      //std::string histogramName_LO_wHHReweighting_full = Form("%s/%s/%s", directoryName.data(), subdirectoryName_LO_wHHReweighting.data(), observable->data());
      //TH1* histogram_LO_wHHReweighting = loadHistogram(inputFile_LO_wHHReweighting, histogramName_LO_wHHReweighting_full);
      //histogram_LO_wHHReweighting->Scale(sf_LO_wHHReweighting);
      //double integral_LO_wHHReweighting = compIntegral(histogram_LO_wHHReweighting);

      std::string histogramName_NLO_full = Form("%s/%s/%s", directoryName.data(), subdirectoryName_NLO.data(), observable->data());
      ...
      std::string outputFileName = Form("plots/compareGenKinematics_HH_%s_%s.png", observable->data(), period->data());
