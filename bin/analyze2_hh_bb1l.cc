...
//--- read python configuration parameters
  ...
  bool usejpa = cfg_analyze.getParameter<bool>("usejpa");
  ...
  if ( fillHistograms_LBN )
  {
    ...
    LBN_resonant_spin2_boosted = makeTensorFlowInterfaceLBNMap(cfg_LBN_resonant_spin2_boosted, era_string,
        false, true, split_resonant_training);
    ...
    LBN_resonant_spin0_boosted = makeTensorFlowInterfaceLBNMap(cfg_LBN_resonant_spin0_boosted, era_string,
        true, false, split_resonant_training);
    ...
    LBN_nonresonant_resolved = makeTensorFlowInterfaceLBNMap(cfg_LBN_nonresonant_resolved, era_string,
        false, false, false);
    // For nonresonant case training is done separately for each BM points while for resonant case
    // one single training is done using genmHH as parameter
    if ( !(fillHistograms_resonant_spin0 || fillHistograms_resonant_spin2) )
      assert( LBN_nonresonant_boosted.size() == 14 && LBN_nonresonant_resolved.size() == 14 );
    if ( fillHistograms_resonant_spin0 )
      assert( LBN_resonant_spin0_boosted.size() == 1 && LBN_resonant_spin0_resolved.size() == 1 );
    if ( fillHistograms_resonant_spin2 )
      assert( LBN_resonant_spin2_boosted.size() == 1 && LBN_resonant_spin2_resolved.size() == 1 );
  }  
...
//--- declare histograms
  ...
  EventCategory_hh_bb1l_LBN eventCategory_LBN( !(fillHistograms_resonant_spin0 || fillHistograms_resonant_spin2) );
  ...
  if ( selectBDT ) {
    ...
    else 
    {
      ...
      bdt_filler->register_variable<float_type>(
        ...
        "mjj_highestpt",
        "met_px", "met_py", "met_pz", "met_e",
        "m_Hww",  "m_HH_bbregCorr", "m_wlep", "dPhi_met_lep", "dR_lep_Wjj", "dR_lep_Hbb",
        "pT_wlep", "dPhi_met_Hbb", "dPhi_met_Wjj"
      );
...
//    to also pass the tight identification and isolation criteria
    ...
    const RecoJetBase* selJet1_jj = nullptr;
    const RecoJetBase* selJet2_jj = nullptr;
    JPA jpa;
    if ( usejpa )
    {
      if ( selJetAK8_Hbb )
    ...
    else
    {
      ...
      if ( cleanedJetsAK4_wrtHbb.size() >1 )
      {
       selJet2_Wjj = cleanedJetsAK4_wrtHbb[1];
      }
      if ( cleanedJetsAK4_wrtHbb.size() >2 )
      {
        selJet1_jj = cleanedJetsAK4_wrtHbb[2];
      }
      if ( cleanedJetsAK4_wrtHbb.size() >3 )
      {
        selJet2_jj = cleanedJetsAK4_wrtHbb[3];
      }
    }
    ...
    std::vector<const RecoJetBase*> selJets_jj;
    if ( selJet1_Wjj ) selJets_Wjj.push_back(selJet1_Wjj);
    if ( selJet2_Wjj ) selJets_Wjj.push_back(selJet2_Wjj);
    if ( selJet1_jj ) selJets_jj.push_back(selJet1_jj);
    if ( selJet2_jj ) selJets_jj.push_back(selJet2_jj);
...
//--- compute MHT and linear MET discriminant (met_LD)
    ...
    double HT = ( selJetsAK8.size()==0 ) ? compHT({}, {}, selJetsAK4) :
      compHT({}, {}, selJetsAK8, cleanedJetsAK4_wrtselJetsAK8);
    ...
    dR_lep_Hbb = deltaR(selLeptonP4, HbbP4);
    double dPhi_met_lep = TMath::Abs(deltaPhi(selLeptonP4.phi(), metP4.phi()));
    double dPhi_met_Hbb = TMath::Abs(deltaPhi(HbbP4.phi(), metP4.phi()));
    ...
    double dR_lep_Wjj = deltaR(selLeptonP4, WjjP4);
    double tau21_Wjj = ( selJetAK8_Wjj ) ? selJetAK8_Wjj->tau2()/selJetAK8_Wjj->tau1() : -1.;
    m_HH_bbregCorr = (HbbP4_reg + WjjP4 + selLeptonP4 + metP4).mass();
    double dR_Hww = -1.;
    double dPhi_Hww = -1.;
    ...
    m_Hww = HwwP4.mass();
    double dPhi_met_Wjj = TMath::Abs(deltaPhi(WjjP4.phi(), metP4.phi()));
    double m_wlep = (selLeptonP4 + metP4).mass();
    double pT_wlep = (selLeptonP4 + metP4).pt();
    ...
    double m_HH_analytic = comp_mHH_analytic(HHvisP4, metP4);
    ...
    double avg_dr_jet_central = comp_avg_dr_jet(selJetsAK4);
    ...
    const std::map<std::string, Particle> ll_inputs = {
      ...
      { "jet1",  Particle(selJetP4_jj_lead)    },
      { "jet2",  Particle(selJetP4_jj_sublead) },
    ...
    std::map<std::string, double> mvaInputVariables_list = {
      ...
      {"jet1_e",               selJetP4_jj_lead.energy()},
      {"jet1_px",              selJetP4_jj_lead.px()},
      {"jet1_py",              selJetP4_jj_lead.py()},
      {"jet1_pz",              selJetP4_jj_lead.pz()},
      {"jet2_e",              selJetP4_jj_sublead.energy()},
      {"jet2_px",             selJetP4_jj_sublead.px()},
      {"jet2_py",             selJetP4_jj_sublead.py()},
      {"jet2_pz",             selJetP4_jj_sublead.pz()},
      {"met_px",                  metP4.px()},
      {"met_py",                  metP4.py()},
      {"met_pz",                  metP4.pz()},
      {"met_e",                  metP4.energy()},
      ...
      {"isVBF",                   isVBF},
      {"m_Hww",                   m_Hww},
      {"m_HH_bbregCorr",          m_HH_bbregCorr},
      {"m_wlep",                  m_wlep},
      {"dPhi_met_lep",            dPhi_met_lep},
      {"dR_lep_Wjj",              dR_lep_Wjj},
      {"dR_lep_Hbb",              dR_lep_Hbb},
      {"pT_wlep",                 pT_wlep},
      {"dPhi_met_Hbb",            dPhi_met_Hbb},
      {"dPhi_met_Wjj",            dPhi_met_Wjj}
    };
...
//--- compute event-level LBN outputs
    ...
    std::map<std::string, double> lbnOutputs_resonant_spin0_400_overlap;
    std::map<std::string, double> lbnOutputs_resonant_spin0_450_overlap;
    std::map<std::string, double> lbnOutputs_resonant_spin2_400_overlap;
    std::map<std::string, double> lbnOutputs_resonant_spin2_450_overlap;
    if ( fillHistograms_LBN )
    {
      if ( selJetAK8_Hbb )
      {
        ...
        {
          hl_inputs_resonant_spin0["gen_mHH"] = 400;
          lbnOutputs_resonant_spin0["400_overlap_spin0"] = (*LBN_resonant_spin0_boosted["spin0_high"])(ll_inputs_ptr, hl_inputs_resonant_spin0, eventInfo.event);
          hl_inputs_resonant_spin2["gen_mHH"] = 400;
          lbnOutputs_resonant_spin2["400_overlap_spin2"] = (*LBN_resonant_spin2_boosted["spin2_high"])(ll_inputs_ptr, hl_inputs_resonant_spin2, eventInfo.event);
          hl_inputs_resonant_spin0["gen_mHH"] = 450;
          lbnOutputs_resonant_spin0["450_overlap_spin0"] = (*LBN_resonant_spin0_boosted["spin0_high"])(ll_inputs_ptr, hl_inputs_resonant_spin0, eventInfo.event);
          hl_inputs_resonant_spin2["gen_mHH"] = 450;
          lbnOutputs_resonant_spin2["450_overlap_spin2"] = (*LBN_resonant_spin2_boosted["spin2_high"])(ll_inputs_ptr, hl_inputs_resonant_spin2, eventInfo.event);
        }
      }
      else
      {
        ...
        lbnOutputs_resonant_spin2 = CreateResonantLBNOutputMap(gen_mHH, LBN_resonant_spin2_resolved, ll_inputs_ptr, hl_inputs_resonant_spin2, eventInfo.event, "_spin2", split_resonant_training);
        ...
        lbnOutputs_resonant_spin0 = CreateResonantLBNOutputMap(gen_mHH, LBN_resonant_spin0_resolved, ll_inputs_ptr, hl_inputs_resonant_spin0, eventInfo.event, "_spin0", split_resonant_training);
        ...
        {
          hl_inputs_resonant_spin0["gen_mHH"] = 400;
          lbnOutputs_resonant_spin0["400_overlap_spin0"] = (*LBN_resonant_spin0_resolved["spin0_high"])(ll_inputs_ptr, hl_inputs_resonant_spin0, eventInfo.event);
          hl_inputs_resonant_spin2["gen_mHH"] = 400;
          lbnOutputs_resonant_spin2["400_overlap_spin2"] = (*LBN_resonant_spin2_resolved["spin2_high"])(ll_inputs_ptr, hl_inputs_resonant_spin2,eventInfo.event);
          hl_inputs_resonant_spin0["gen_mHH"] = 450;
          lbnOutputs_resonant_spin0["450_overlap_spin0"] = (*LBN_resonant_spin0_resolved["spin0_high"])(ll_inputs_ptr, hl_inputs_resonant_spin0,eventInfo.event);
          hl_inputs_resonant_spin2["gen_mHH"] = 450;
          lbnOutputs_resonant_spin2["450_overlap_spin2"] = (*LBN_resonant_spin2_resolved["spin2_high"])(ll_inputs_ptr, hl_inputs_resonant_spin2,eventInfo.event);
        }
      }
    }
      ...
      for (const GenMatchEntry* genMatch : genMatches)
      {
        ...
        if(! skipFilling)
        {
          ...
          selHistManager->evt_->fillHistograms(
            ...
            dPhi_Hww, pT_Hww,
            m_HH_B2G_18_008, m_HH_analytic, m_HHvis, m_HH, pT_HH, dPhi_HHvis, pT_HHvis,
            mT_W, mT_top_2particle, mT_top_3particle,
