...
class prodNtupleConfig_hh_bbwwMEM_dilepton(analyzeConfig_hh):
  """Configuration metadata needed to run analysis in a single go.

  Sets up a folder structure by defining full path names; no directory creation is delegated here.

  Args specific to prodNtupleConfig_hh_bbwwMEM_dilepton:
    None.
  ...
  def createCfg_analyze(self, jobOptions, sample_info):
    """Create python configuration file for the produceMEMNtuple_hh_bb2l executable (analysis code)
    ...
    lines = super(prodNtupleConfig_hh_bbwwMEM_dilepton, self).createCfg_analyze(jobOptions, sample_info,
  ...
  def create(self):
    ...
    for sample_name, sample_info in self.samples.items():
      ...
      numJobsPerFile = None
      if   sample_info["process_name_specific"].find("signal_ggf_nonresonant_node_sm_hh_2b2v") != -1:
        numJobsPerFile = 500
      elif sample_info["process_name_specific"].find("signal_ggf_nonresonant_cHHH1_hh_2b2v") != -1:
        numJobsPerFile = 100
      elif sample_info["process_name_specific"].find("TTJets_DiLept") != -1:
        numJobsPerFile = 50
      elif sample_info["process_name_specific"].find("TTTo2L2Nu") != -1: 
        numJobsPerFile = 10
      else:
